...
// v3: 名前どおり実際に「呼吸」させる — 球は深呼吸ペース（吸う/吐く 各2.6秒）で
// ゆっくり拡縮し、ハローは同じ位相で明滅・膨張する。
const STACK = 230;
const ORB = 168;
const C = ORB / 2;
...
export function SleepOrb({ onPress }: { onPress: () => void }) {
  ...
  const orbScale = breath.interpolate({ inputRange: [0, 1], outputRange: [1, 1.04] });
  const ringScale = breath.interpolate({ inputRange: [0, 1], outputRange: [1, 1.055] });
  const ringOpacity = breath.interpolate({ inputRange: [0, 1], outputRange: [0.5, 1] });

  return (
    <View style={styles.stack}>
      {/* 同心の呼吸ハロー（淡い輪・球より少し大きく膨らむ） */}
      <Animated.View
        style={[styles.ring, styles.ring1, { opacity: ringOpacity, transform: [{ scale: ringScale }] }]}
        pointerEvents="none"
      />
      <Animated.View
        style={[styles.ring, styles.ring2, { opacity: ringOpacity, transform: [{ scale: ringScale }] }]}
        pointerEvents="none"
      />
      <Animated.View style={{ transform: [{ scale: orbScale }] }}>
        <Pressable
          ...
          accessibilityLabel="おやすみ。録音をはじめる"
